...
public sealed class BlazorFormValidator
{
        ...
        List<BlazorFormValidationMessage> messages)
    {
        ...
        foreach (var rule in field.Validators)
        {
            ...
            var ctx = new BlazorFormValidationContext(path, value, data, services);
        ...
        if (field.Type == BlazorFormFieldType.Object)
        {
            foreach (var child in field.Children)
                await ValidateField(child, BlazorFormPath.Combine(path, child.Name), data, services, includeAsync, messages);
        }
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (template.Type == BlazorFormFieldType.Object)
                {
                    foreach (var child in template.Children)
                        await ValidateField(child, BlazorFormPath.Combine(itemPath, child.Name), data, services, includeAsync, messages);
                }
                else
                {
