...
namespace BlazorForm.Core.State;

/// <summary>
/// Optional external validator (e.g. FluentValidation) invoked alongside the schema's built-in rules.
/// </summary>
public delegate ValueTask<IReadOnlyList<ValidationMessage>> ExternalValidator(
    FormDefinition form, IFormDataReader data, IServiceProvider? services);
...
public sealed class FormState
{
    ...
    private readonly Dictionary<string, List<ValidationMessage>> _messages = new(StringComparer.OrdinalIgnoreCase);
    ...
    public FormState(FormDefinition definition, IFormDataAccessor data, IServiceProvider? services = null)
    {
    ...
    public IFormDataAccessor Data { get; }
    ...
    public ExternalValidator? ExternalValidator { get; set; }
    ...
    public bool IsVisible(FieldDefinition field)
    ...
    public bool IsDisabled(FieldDefinition field)
    ...
    public bool IsStepVisible(FormStep step)
    ...
    public IEnumerable<ValidationMessage> AllMessages => _messages.Values.SelectMany(x => x);

    /// <summary>Messages for a specific field path.</summary>
    public IReadOnlyList<ValidationMessage> MessagesFor(string path)
        => _messages.TryGetValue(path, out var list) ? list : Array.Empty<ValidationMessage>();

    /// <summary>True if there is at least one error-severity message.</summary>
    public bool HasErrors => AllMessages.Any(m => m.Severity == ValidationSeverity.Error);
    ...
    public async ValueTask<bool> ValidateStepAsync(bool includeAsync = true)
    {
        ...
        foreach (var m in messages)
            AddMessage(m);
        NotifyChanged();
        return !messages.Any(m => m.Severity == ValidationSeverity.Error);
    }

    /// <summary>Validates a single field and refreshes only its messages.</summary>
    public async ValueTask ValidateFieldAsync(FieldDefinition field, string path, bool includeAsync = true)
    {
    ...
    {
        AddMessage(new ValidationMessage(path, message));
        _touched.Add(path);
        NotifyChanged();
    }
    ...
    public FormStep? CurrentStep => Definition.IsWizard ? Definition.Steps[CurrentStepIndex] : null;
    ...
    public int AddArrayItem(FieldDefinition arrayField, string arrayPath)
    {
    ...
    private async ValueTask<IReadOnlyList<ValidationMessage>> MergeExternal(IReadOnlyList<ValidationMessage> messages)
    {
    ...
    private void ReplaceAllMessages(IReadOnlyList<ValidationMessage> messages)
    {
        _messages.Clear();
        foreach (var m in messages) AddMessage(m);
    }
    ...
    private void AddMessage(ValidationMessage message)
    {
        if (!_messages.TryGetValue(message.FieldPath, out var list))
            _messages[message.FieldPath] = list = new List<ValidationMessage>();
        list.Add(message);
    }
    ...
    private void ApplyDefault(FieldDefinition field, string path)
    {
        ...
        if (field.Type == FieldType.Object)
            foreach (var child in field.Children)
                ApplyDefault(child, FormPath.Combine(path, child.Name));
    }
    ...
    private IList EnsureList(string arrayPath)
    {
        ...
        IList newList;
        if (Data is DictionaryDataAccessor)
        {
    ...
    private object? CreateItem(FieldDefinition arrayField, string arrayPath)
    {
        var template = arrayField.ItemTemplate;
        var elementType = Data.GetElementType(arrayPath);

        if (Data is DictionaryDataAccessor || elementType is null || elementType == typeof(object))
            return template?.Type == FieldType.Object ? new Dictionary<string, object?>() : null;
