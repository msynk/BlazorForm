...
/// </summary>
public sealed class ModelDataAccessor : IFormDataAccessor
{
    public ModelDataAccessor(object model) => Root = model ?? throw new ArgumentNullException(nameof(model));
    ...
    private static object? ReadSegment(object target, PathSegment seg)
    {
        if (seg.IsIndex)
        {
    ...
    private static void WriteSegment(object target, PathSegment seg, object? value)
    {
        if (seg.IsIndex)
        {
    ...
    private static object CreateChild(object parent, PathSegment seg)
    {
        if (seg.IsIndex)
